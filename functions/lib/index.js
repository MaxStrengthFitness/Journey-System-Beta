...
exports.setCustomUserClaims = exports.calculateFacilityAnalytics = exports.issueMindbodyUserToken = void 0;
const functions = require("firebase-functions/v1");
const admin = require("firebase-admin");
var issueUserToken_1 = require("./mindbody/issueUserToken");
Object.defineProperty(exports, "issueMindbodyUserToken", { enumerable: true, get: function () { return issueUserToken_1.issueMindbodyUserToken; } });
admin.initializeApp();
 ...
 */
exports.calculateFacilityAnalytics = functions.pubsub
    .schedule("0 2 * * *")
    .timeZone("Etc/UTC")
    .onRun(async (context) => {
...
async function calculateAndSaveFacilitySummary() {
    ...
    const clients = clientsSnap.docs.map((d) => (Object.assign({ id: d.id }, d.data())));
    const sessions = sessionsSnap.docs.map((d) => (Object.assign({ id: d.id }, d.data())));
    const logs = logsSnap.docs.map((d) => (Object.assign({ id: d.id }, d.data())));
    ...
    const cohortMap = clients.reduce((acc, client) => {
    ...
    const retention = Object.values(cohortMap).map((cohort) => ({
    ...
    const machineEfficacy = machineIds.map((machineId) => {
        ...
        const avgTimeUnderLoad = timeSpentArray.length > 0
    ...
    const averageAggregateIncrease = machineEfficacy.length > 0
        ? machineEfficacy.reduce((acc, m) => acc + m.percentIncreaseOperationalLoad, 0) / machineEfficacy.length
        : 0;
...
exports.setCustomUserClaims = functions.https.onCall(async (data, context) => {
    ...
    const { uid, role } = data;
    if (!uid || !role) {
        throw new functions.https.HttpsError("invalid-argument", "Both 'uid' and 'role' fields are required.");
    }
    const allowedRoles = ["Admin", "Founder", "Overseer", "FranchiseOwner", "Owner", "StudioOwner", "HeadTrainer", "Trainer", "LifeTransformer"];
    if (!allowedRoles.includes(role)) {
        throw new functions.https.HttpsError("invalid-argument", `The role '${role}' is not a valid organization role.`);
    }
    ...
    if (!isAuthorized) {
        ...
        if (systemConfigSnap.exists) {
            const adminUids = ((_b = systemConfigSnap.data()) === null || _b === void 0 ? void 0 : _b.adminUids) || [];
    ...
    if (!isAuthorized) {
        throw new functions.https.HttpsError("permission-denied", "You are not authorized to assign organization claims.");
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", `Error assigning user claims: ${error.message}`);
    }
});
