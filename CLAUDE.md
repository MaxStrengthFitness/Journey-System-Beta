...
| Typecheck | `npx tsc --noEmit` | Compare the error **count** to the baseline - **11** on `operations-overhaul` (Sep 19 2026, unchanged from `master` after the beta-prep merge), measured in a clean clone in the container; don't expect zero. History by round: `docs/KNOWN-TRAPS.md#baselines` |
| Tests | `npx vitest run src` | **3,469** passing in 232 files (one skipped) on `operations-overhaul` (Sep 19 2026); 3,396 in 224 on `master` after the Machine Trends panel. Run it as `TZ=America/New_York npx vitest run src` - see the date trap. History by round: `docs/KNOWN-TRAPS.md#baselines` |
