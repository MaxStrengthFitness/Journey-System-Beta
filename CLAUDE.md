...
| Tests | `npx vitest run src` | **3,516** passing in 237 files (one skipped) on `machine-authoring` (Sep 20 2026); 3,469 in 232 on `operations-overhaul`. Run it as `TZ=America/New_York npx vitest run src` - see the date trap. History by round: `docs/KNOWN-TRAPS.md#baselines` |
