...
| Tests | `npx vitest run src` | 2,128 passing after the cost round (Sep 16) — run it as `TZ=America/New_York npx vitest run src`, see the date trap below; 2,077 after the four-tab profile round; 2,046 after the FORD round; 2,027 after the fix round; 2,015 after the tracker round; 1,813 after the floor round |
...
## Known traps (as of Sep 16 2026, after the cost round)
...
- **`useClientJournal`'s four unbounded collections carry a 200-item guard rail, unordered on purpose** (`JOURNAL_GUARD_LIMIT`). Don't turn it into a "newest N": the focus board counts every past focus and offers a new one only when none is active, so an ordered cut would duplicate focuses and miscount. When the rail is hit the hook says `capped` and the journal tab says so.
...
- On AJ's PC, Claude's Linux shell can't reach the project folder (a Windows update on Sep 8 2026 stopped it starting); the file bridge (stage and commit files) works, and rounds ship as patches + a staged script. The cloud container **can** `npm ci`, typecheck, run the suite and both builds (corrected Sep 16); it cannot run `test:rules` (the emulator jar's host is blocked), so **AJ's `test:rules` run is the one that counts.**
