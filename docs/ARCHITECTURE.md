...
### 2.5 The Operations dashboard — sixteen tabs, one scope

Reaching it: the App Mode toggle (studio leaders and above) or the studio picker's "Go to Operations" (the same `isStudioLeader` gate since Sep 19 — the hard-coded e-mail is gone). Inside, `isFranchiseOwnerOrAdmin` (admin, or the role codes `FranchiseOwner` / `Owner` — not `StudioOwner`, a studio-tier role labelled Studio Owner since Sep 18) gates some tabs and `isAdmin` gates the System Backend group; the Firestore rules are the real enforcement.
...
| --- | --- | --- | --- | --- |
| Studio Management | Monday | `features/admin/monday` `MondayPage` — the four questions (§1.5), today's floor, one line each to Delight, Insights, Machine fit and Hours | everyone on the screen | the network view (`features/admin/network`) |
| | Renewals | `features/admin/renewals` — Pipeline and Outcomes; the settings are My Studio → Studio, and this tab points there with the count of unmatched Mindbody names | everyone | pick a studio |
| | Delight | `features/ford/DelightQueue` — the studio's gestures with row actions (Take it · Hand it to… · Done · Pass), the Passed bucket, show done | everyone | pick a studio |
| | Staff & Roles | `features/admin/staff` | franchise owner or admin — `useStaffRoster` + `StaffEditor`, shared with My Studio → Team | spans |
| | Clients | `features/admin/clients` | everyone | spans (an `in` clause) |
| | All locations | `features/admin/studios` — the registry: create, delete, franchises, the Mindbody audit; the same `StudioDetailsForm` My Studio → Studio renders | franchise owner or admin | spans |
| | Catalog | `components/machines/AdminMachinesTab` — the MSF standard set as a view (`features/admin/catalog/StandardSetPanel`), the studios' submissions queue (`SubmissionsQueue`, administrators), the machine editor | franchise owner (reads) or admin | global |
| | Routines | `components/routines/AdminRoutineTemplatesTab` | everyone (authoring gated inside) | readable studios listed |
| | Insights | `features/admin/insights` — says "could not be loaded" on a refused read | everyone | pick a studio |
| | Machine fit | `features/admin/machine-fit` | everyone — **This studio**, live; the **All MSF studios** scope inside it is admin only | pick a studio |
| | Hours | `features/admin/hours` — booked slots × the studio's session length, by trainer, week and month | everyone | spans, with the company total |
| | Exports | `features/admin-data` — Sessions by trainer, Attendance (the legacy importer is gone) | franchise owner or admin | pick a studio |
| Communications | Announcements | `features/admin/announcements` — owners and admins; Take down confirms | franchise owner or admin | — |
| System Backend | Mindbody | `features/admin/mindbody` | admin | — |
| | Limbo | `components/AdminLimboQueue` | admin | pick a studio |
| | Bug Reports | `features/admin/bugs` | admin | — |
| | System Tools | `components/AdminSystemToolsTab` | admin | — |

"Overview", "Retention", "Integrations" and the Franchise screen are gone (the first replaced by the Monday page, the second deleted, the third folded into Mindbody, the fourth folded into the scope).
