...
| `studio-tasks` | **Planner** (`features/planner/PlannerView.tsx`) | 4 | Studio hub (at-a-glance band, shift strip, team jobs, clients waiting, the board, the playbook) · My tasks (with reminders and team jobs) · Notes (with colleague shares) · Team (leaders of this studio) — Planner rework, Sep 16 | `studios/{s}/taskTemplates`, `taskInstances`, `taskRequests` (+ `submissions`), `teamJobs`, `noteShares`, `playbook`; `trainers/{uid}/taskTemplates`, `taskInstances`, `notes`, `noteFolders`; `clients/{id}/sharedNotes` (per README) | the same paths, plus `trainers/{uid}/notifications` (job, share and reminder bells) | profile, progress-report, consultation-wizard |
