...
| ~~`consultation-wizard`~~ | ~~Consultation wizard~~ | — | **Route removed Sep 24 2026.** Its only door was the Relay client task "Assessment", which is the **Pulse** task now and opens the profile at Notes & Profile → Body & Pulse (the stored key is still `assessment`). `components/ConsultationWizard.tsx` stays, unmounted, for the consultation redesign; its Male / 40 defaults were removed the same day. | — | — | — |
...
### 2.3 The client profile — four tabs (seven until Sep 15; Notes & Profile as seven pages since Sep 24)
...
**Notes & Profile — the client codex** (`src/features/client-codex/README.md`; the round is `docs/rounds/2026-09-24-client-codex.md`). One sub-toggle of seven segments that wraps rather than clips, one load for the whole tab (the journal with its 40 newest sessions, FORD, the InBody scans, the trainer's note dismissals, the Pulse history from the profile's progress-reports listener), one form and one Save bar for every record field a page edits, and read-only pages for a reader the rules would refuse (a cross-train studio):

| Page | Component | Shows |
| --- | --- | --- |
| **Overview** | `client-codex/pages/OverviewPage` | Six slots from the selectors the pages behind them use: the loudest open notes, who she is (In one line, the four pillars, Coming up, the gestures), body and Pulse, goals, the story so far, the account. Zero reads |
| **Notes** | `client-notes/NotesPage` | Every note as a thread — Open, Standing context, Resolved — with the composer (FORD / Life saves to FORD in place), the To-file tray and one filter row |
| **FORD** | `ford/page/FordPage` | In one line, Coming up, To file, the four pillars (band, facts, older life notes, the Pulse beside it, Ask next and Follow up next time), above and beyond |
| **Body & Pulse** | `client-codex/body/BodyPulsePage` | Build (height, reach, weight, the Training story), where it matters on the figure, the watch-outs quoted from the clinical matrix, her machines on our floor, measured beside told, Over time (how she arrives and leaves, the Pulse, InBody), the Pulse, the InBody card |
| **Goals & Focus** | `goals/GoalsPage` | How to coach her, her why, what she is working toward, each coach's focus, the team's plans and your jots, what she has reached |
| **Story** | `client-story/StoryPage` | Her time with Max Strength as dated moments, with FileMaker's sessions before Journey counted and never called new |
| **Account** | `client-admin/AccountPage` | Contact, the Mindbody account notes with the intake matcher, what is on file, the membership and its contracts, where she trains, how she found us, the fine print (Migration Hub) |

The header (`ProfileHeader`, `features/client-profile`): Back → client directory; **Start Session** (the hero action) → `workouts`; when a session is already In-Progress the same slot becomes Continue (the trainer's own) or Watch (another trainer's, read-only, with Take over on that screen) / Discard (session record, Sep 26 2026); the Kaizen toggle adds or removes the client from the trainer's roster; the package tile opens the renewal card. An alert strip above the header ("Report Required" / "Report Due") jumps to the check-in editor.

The two hidden dead panes this section once listed (`statistics_disabled`, `details_disabled`) were deleted in the profile merge (Sep 15).

### 2.4 The Active Session — the Rank 1 path, step by step

All of it is `currentView === "workouts"` inside `WorkoutTrackerView.tsx`.

1. **Entry.** Profile → Start Session (or Continue), the bottom-nav Start Session, or an open session with no client (`startUnassignedSession`). While a client is selected the screen streams `clientMachineSettings`, `routines`, `sessions` by client (carrying on with a live In-Progress one when it is this trainer's; another trainer's opens read-only in `WatchingSession`, with Take over, session record, Sep 26 2026), `sessionNotes`, `focusRecords`, and `exerciseLogs` for up to 30 sessions; the studio's floor comes from `useStudioMachines` (`studios/{s}/roster` + `machines`).
...
- ~~`ClientProfileView`'s two hidden panes (`statistics_disabled`, `details_disabled`) with live handlers inside~~ — deleted in the profile merge (Sep 15 2026). The long scroll that replaced them (`ClientInfoSheet` → `ClientDossier`) went in the client codex (Sep 24 2026), and so did the old journal area (`ClientJournalTab`), the dossier's form primitives and its journal rails.
...
| `studios/{id}` | `Studio`: owner and head trainer ids, timezone, Mindbody site and location ids, `journeyCutoverDate`, shift hours, `sessionMinutes` (the booked slot Hours counts, default 30 — Sep 19), `inbodyVariation` (the smallest InBody change any screen may call a change, per measure; absent = Max Strength's defaults 3.5 lb / 5.3 lb / 2.7 points; written whole with `updatedBy` = Auth uid on My Studio → Studio; read through `features/inbody/variation.ts` for the client's HOME studio by the InBody card, Body & Pulse's Strength pair and its Over time band, the progress report, the Renewal Brief, the renewal card and the pipeline — client codex, Sep 24), sync lease, `networkId` | config | any signed-in user | create: admins, franchise owners; update: those plus **the studio's own leaders** (a leader role there or the grant); any trainer may write only the sync lease (`lastScheduleSyncAt`, `scheduleSyncFailures`) — My Studio, Sep 19 | canonical; the any-trainer write hole is closed |
...
| Path | Holds | Owner | Read | Write | Status |
| --- | --- | --- | --- | --- | --- |
| `clients/{mindbodyClientId}` (or `{siteId}-{mindbodyClientId}` for the second person on a number both Mindbody sites use — `lib/mindbody-site.ts`) | `Client`: Mindbody mirrors, coaching fields (incl. the optional `wingspan`, machine-fit round; `recoveryMetric` is retired from every screen — kept on old records, nothing reads or writes it, client codex, Sep 24), running totals, `renewal`, `inbodySummary`, `subjectiveSnapshot` | Mindbody (identity, commercial) · Journey (coaching) · job (`renewal`) | admins, franchise owners, trainers and leaders of the home studio, approved cross-train studios | create/update by admins or the home studio's trainers and leaders, never touching `renewal`; delete admins | canonical |
| `clients/{id}/ford/{id}` | `FordEntry` — one personal detail (Family, Occupation, Recreation, Dreams, or none yet), with an optional date, gesture (`opportunity`) and, since the client codex, **Follow up next time** (`followUp`, `followUpAt`, `followUpBy`: at most 140 characters, stamped only when the question changes). Stamped with the client's studio (`fordStudioIdOf`: `homeStudioId`, else the older `studioId`). **`clients/{id}/ford/one-line`** is not a detail but **In one line**, the team's sentence about her (a fixed id, `kind: "one-line"`, stored `isArchived: true` with no pillar so every reader of details skips it; 120 characters) | Journey | trainers and leaders of the stamped studio (the grant counts), franchise owners, administrators — **never a cross-train studio**; every per-client query filters `studioId` (the rule tests it; rules are not filters) | create: a trainer of the stamped studio, as author (Auth uid) — no administrator or franchise clause; update: the same plus administrators and franchise owners, `clientId` and `studioId` immutable; delete: administrators, franchise owners, or the author of a pillar-less document | canonical. The collection group feeds the Delight queue. Read once per profile by the codex (`useClientFord`); `fordSummary` on the client is a best-effort cache of counts and dates (and still of `pinned` lines — held for AJ) |
...
| Sep 23 2026 | **The client codex** (AJ's answers to the ten questions of the Client Codex mockup): Notes & Profile opens on an Overview, then Notes · FORD · Body & Pulse · Goals & Focus · Story · Account in that order; keep the build-as-the-machines-see-it and the body figure (never a body-type label); move Experience to Body & Pulse; YES to two new trainer fields, **In one line** and **Follow up next time** (his Firestore OK for them); YES to the Mindbody intake-notes matcher (nothing copied without a tap; a medical line is never judged covered); retire the "Recovery between sessions" dropdown from the screen (the data stays); **an InBody change smaller than the scanner's normal variation is not called a change, and each studio sets its own variation** (Max Strength's defaults otherwise); show how she arrives at each session (from the sessions the journal already streams — no new read); build it on the `client-codex` branch. Built in nineteen phases (`docs/rounds/2026-09-24-client-codex.md`); **no rules or index change** |
...
| Sep 24 2026 | **A Critical note marks the Hub card** (AJ, question 12 of the Sep 20 audit). **The red triangle only** — the left edge keeps its meanings and no note turns it red. **From the day's notes, never a count on the client**: one live read of the Critical notes of the clients booked on the day on screen, thirty to a query, by client (writers stamp different studios), and the briefing's rule asked about the booking's day (`src/lib/hub-critical-notes.ts`). **Red for every trainer**: the Hub ignores "No need to remind me", like the record's critical line. `docs/rounds/2026-09-24-hub-critical-flag.md` |
