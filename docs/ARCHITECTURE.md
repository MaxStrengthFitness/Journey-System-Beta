...
- **The three deferred write holes revisited and closed** (§5.3; decided Sep 12 2026: Gate C, right before the first franchisee goes live on the app) — a trainer editing their own role and studio lists, any trainer editing any `studios/{id}`, any trainer writing another studio's task instances and requests. The partition itself is a Gate B item since Sep 12.
