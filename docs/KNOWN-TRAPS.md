...
- **A cross-train visitor could read the client but not finish her session (Sep 24 2026, packages round).**
  ...
  a History delete write, and nothing else (`crossTrainSessionTotals`, an
  allowlist of sixteen fields; her name, her home, her approvals, her medical
  history and `renewal` stay refused), and `completeWorkoutSession`
  (`src/lib/sync-utils.ts`) now commits the session, sets and settings first
  and writes the totals after it on their own, so any refusal of the totals
  costs the totals and says so (`totalsSaved: false`, a toast), never the
  session. **A write that must never fail does not share a batch with one a
  rule may refuse. A read grant is not a write grant: when you open a
  document to another studio, list what that studio's screens write to it
  and test each one.** Tests: `a cross-train visitor's session` in
