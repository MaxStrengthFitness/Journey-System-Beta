...
- **The fluidity round (Sep 17–18) — read `docs/rounds/2026-09-17-fluidity-audit.md` and `docs/rounds/2026-09-18-floor-audit.md`.** The load-bearing parts:
  ...
  - **Every claim about a client's PAST goes through `src/lib/history-claims.ts`** (prior-history sweep, Sep 24 2026 — `docs/rounds/2026-09-24-prior-history-claims.md`). "First time", "#N", "Completed sessions", a break, "Total Sessions", "All time", "Start of history", "Client since": each has one function there, and every one defaults to the cautious wording. Coverage comes from `coverageOfClient` with the client's **HOME** studio's cutover (`homeCutoverOf`, or `useClientCoverage` inside the studio context) — never `activeStudio`, which is the iPad's. **`client.firstSessionDate` is the client's first session IN JOURNEY** (the Active Session stamps it), not her first session ever — which is why "Client since" takes the earliest of it and Mindbody's dates. **A gap is a break only inside `ownedWindow`**: FileMaker stays live through the migration. **Crimson is never used for absence.**
