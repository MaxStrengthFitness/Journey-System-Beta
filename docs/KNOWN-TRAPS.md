...
- **The client profile's record tab is the whole non-training record — as seven pages since the client codex** (phase 8, Sep 24 2026). The long scroll (`ClientInfoSheet` → `ClientDossier`, eight sections, a jump rail) is gone: Notes & Profile is `features/client-codex/` — `ClientCodex` and an Overview plus Notes · FORD · Body & Pulse · Goals & Focus · Story · Account, the page and card coming from the nav (`recordPage`, `recordAnchor`). `DossierSection` and `sectionForEntry` stay (journal entries may carry `profileSection`), and every old section id still lands through `SECTION_TO_PAGE`. The Pulse composes on Body & Pulse; the filed shelf is still Activity Archive → Reports. `ClientJournalTab` is mounted by nothing any more (its Notes area went in phase 9 — Notes is `client-notes/NotesPage`, which takes the journal as a REQUIRED prop and has no fallback hook; its Pulse in phase 12; its focus board in phase 14 — Goals & Focus mounts `FocusBoard` itself, on the tab's journal, with the writes in `goals/useFocusActions`), and the cleanup phase deletes it — the codex loads `useClientJournal` ONCE (`useCodexData`) and hands it to every page, so don't add a second hook on a page.
...
- **Never type a raw control or invisible character into source** (a NUL, U+F8FF): write the escape (`\u0000`, `\uf8ff`). A raw NUL makes git treat the file as binary, and a binary diff can't ship as a patch. The usual way one gets in is a **no-break space (U+00A0)** \u2014 it looks like a space everywhere \u2014 and Claude's file-editing tools can DECODE a `\u00a0` written in an edit into the raw character (client codex, phase 5, Sep 24 2026: three test strings came out raw). After writing an escape, check the bytes (`sed -n 'N,Mp' file | cat -A`). The codex's `kit/scale.test.ts` scans every codex source and test file for them.
