...
- **The reporting round (Sep 16) — read `docs/rounds/2026-09-16-reporting-round.md` and `src/features/rating/scales.ts` before touching anything a trainer rates or notes.** The load-bearing parts:
  ...
  - **One `useCheckInDraft` per client per screen** (client codex, phase 6, Sep 24 2026). A screen that shows the Pulse AND mounts `ClientCheckInPanel` owns the one draft and passes it as the panel's `draft` prop; the panel's own hook is then disabled. Two hooks on one client each load the history and each autosave — two drafts side by side. The draft handed in must be for the panel's `client`.
