...
- **The webhook, what was actually wrong (phase 43).** `webhooks-check` showed three subscriptions for the webhook URL — the same three from either site, because subscriptions belong to the developer account, not a site: two `DeactivatedTooManyFailedMessageDeliveryAttempts`, one `PendingActivation`, none with more than 5 events. Mindbody's documentation (developers.mindbodyonline.com/WebhooksDocumentation) settled the rest:
  - It signs as `X-Mindbody-Signature: sha256={base64}`, HMAC-SHA-256 keyed with `messageSignatureKey` **as a UTF-8 string**. `verifySignature.ts` expected no prefix and base64-decoded a 44-character key, so **every real event failed and was answered 401** — hence the deactivations. The only events ever accepted (Aug 7–20, typed `clientUpdated`) came from the app's own test button.
  - It also accepted the literal header `test-signature` from anyone — a way to write to production through the webhook. Removed.
  - `appointmentBooking.updated` does not exist and no `clientContract.*` event is documented; an unknown event id fails the whole request.
  - The signing secret is returned ONLY when a subscription is created, and none was kept.
  So `ship-sep24.ps1 webhooks-on` starts fresh: a new subscription (documented events; contract ids tried, dropped if refused), its secret written only to a temp file that becomes Firebase's `MINDBODY_WEBHOOK_SECRET`, the webhook redeployed with the signature fix, the subscription activated, the dead ones deleted. The server's test button now signs the documented way and refuses without a secret. Note it posts a fake client, `test-client-001`, to the LIVE webhook: don't press it on production.
