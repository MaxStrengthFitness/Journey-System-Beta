...
- The real `ScheduleBlock` was looked at in a throwaway page in the browser
  ...
  clear.
- Not yet seen on an iPad or on the live Hub. Round 19 of
  `docs/ops/TESTING-CHECKLIST.md` is the walkthrough.

## How to ship

Nothing to deploy to Firebase: no index, no rules. Done-means-logged reached
`master` with the landing (Sep 24); this round and the Hub card (its phase 6)
travel on `loose-ends-sep26` (Sep 25), merged onto `lean-sync`.
