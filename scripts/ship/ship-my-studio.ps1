<#
 ...
 WHAT IS ON THE BRANCH
   my-studio carries, on top of master at 93fb096 (the fluidity round), six
   commits - one per phase (docs/rounds/2026-09-19-my-studio.md):
     1  the shell: Relay becomes a section of My Studio, the grant field
     2  the Studio section
     3  the Machines section, Offer to the MSF catalog (catalogSubmissions)
     4  the Team section, the staff editor shared with Staff & Roles
     5  the rules: the grant, studios/{id} written by its own leaders, the
        approval caps, a studio's own notices, catalogSubmissions
     6  docs
   master has nothing the branch lacks, so the merge is a fast-forward.
   No index changes. No Cloud Functions changes. No dependency changes.

 THE RULES ONLY ADD ACCESS - with one closure: a trainer can no longer edit a
 studio document (nothing in the running app does, except the schedule
 sync's lease fields, which the new rule still allows). So the rules go
 first and the running app is unaffected.
 ...
 golive  the deploy order from CLAUDE.md:
           ...
           git checkout master; git merge --ff-only my-studio; git push
         Every step is gated on the one before it. Render deploys on the push.
         Nothing to run afterwards.

 IF check FAILS on the rules tests, read the failing test name: the ten new
 ones are under "MY STUDIO (Sep 19 2026)" at the end of
...
Set-Location $Root
$LogFile = Join-Path $Root 'logs\ship-my-studio.log'
...
Log "ship-my-studio $Stage" 'White'

# ---- where are we -----------------------------------------------------------
$cur = (& git rev-parse --abbrev-ref HEAD).Trim()
if ($cur -ne $Branch) { Log "STOP: on '$cur', expected '$Branch'. git checkout $Branch first." 'Red'; exit 1 }
$dirty = (& git status --porcelain --untracked-files=no -- src docs firestore.rules CLAUDE.md tests) | Where-Object { $_ }
if ($dirty) { Log "STOP: uncommitted changes under src/docs/rules/tests:" 'Red'; $dirty | ForEach-Object { Log "   $_" 'Red' }; exit 1 }
...
if ($Stage -eq 'check') {
  ...
  Log "CHECK PASSED. Next: powershell -ExecutionPolicy Bypass -File .\scripts\ship\ship-my-studio.ps1 golive" 'Green'
  exit 0
}

# ---- golive ------------------------------------------------------------------
Log "golive: rules first (they only add access), then the app." 'White'
...
Log "GOLIVE COMPLETE. master = $((& git rev-parse --short HEAD).Trim()). Watch Render, then walk Round 10 of docs/ops/TESTING-CHECKLIST.md on the iPad." 'Green'
exit 0
