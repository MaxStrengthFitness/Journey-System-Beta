...
export function isLifeTransformer(trainer: Trainer | null): boolean {
  ...
  if (!role) return true;
  return (
    role === "LifeTransformer" || role === "Trainer" || isStudioLeader(trainer)
  );
}
...
export function areStudiosInSameNetwork(
  ...
  return networks.some(
    (net) =>
      net.studioIds.includes(studioIdA) && net.studioIds.includes(studioIdB),
  );
}
...
export function hasPermission(
  ...
  switch (action) {
    ...
    case "delete_trainer":
      ...
      if (isOwner(trainer)) {
        return isStudioInTerritory(
          trainer,
          targetTrainer.primaryHomeStudioId,
          networks,
        );
      }
      ...
      if (isStudioLeader(trainer)) {
        const isStandardTransformer =
          targetTrainer.role === "LifeTransformer" ||
          targetTrainer.role === "Trainer";
        const isSameStudio =
          targetTrainer.primaryHomeStudioId === trainer.primaryHomeStudioId;
    ...
    case "create_client":
      ...
      if (isLifeTransformer(trainer)) {
        return (
          evaluationStudioId === trainer.primaryHomeStudioId ||
    ...
    case "view_reports":
      ...
      const isPermanentAccess =
        trainer.accessibleStudioIds?.includes(clientHome) ||
      ...
      if (evaluationStudioId && evaluationStudioId !== clientHome) {
        const isApprovedForCurrentStudio =
          targetClient.approvedCrossTrainStudioIds?.includes(
            evaluationStudioId,
          );
        if (
          isApprovedForCurrentStudio &&
    ...
    case "train_client":
      return isLifeTransformer(trainer);
