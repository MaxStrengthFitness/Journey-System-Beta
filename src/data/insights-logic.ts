  ...
  public static generateDashboardMetrics(
    ...
    const filteredClients = clients.filter(c => {
      if (filters.ageBrackets && filters.ageBrackets.length > 0) {
        ...
        const match = filters.ageBrackets.some(b => c.age! >= b.min && c.age! <= b.max);
        if (!match) return false;
      }
      if (filters.genders && filters.genders.length > 0) {
        if (!c.gender) return false;
        const match = filters.genders.some(g => c.gender?.toLowerCase() === g.toLowerCase());
        if (!match) return false;
      }
      if (filters.activityLevels && filters.activityLevels.length > 0) {
        const occProfile = OCCUPATIONS.find(o => o.id === c.occupation || o.title === c.occupation);
        const cActivity = occProfile?.activityLevel || c.activityLevel;
        if (!cActivity) return false;
        const match = filters.activityLevels.some(act => 
    ...
    Object.entries(machineEfficacyMap).forEach(([machineId, entry]) => {
      ...
      const mapping = machineMuscleMap[machineId] || machineMuscleMap[`m-${machineId}`];
      let category = 'Core';
      if (mapping && mapping.primary && mapping.primary.length > 0) {
        const p = mapping.primary[0];
        if (['quadriceps', 'gluteal', 'hamstring', 'calves', 'adductor'].includes(p)) {
          category = 'Legs';
        } else if (['chest'].includes(p)) {
          category = 'Chest';
        } else if (['upper-back', 'lower-back', 'trapezius'].includes(p)) {
          category = 'Back';
        } else if (['biceps', 'triceps', 'front-deltoids', 'back-deltoids', 'neck', 'forearm'].includes(p)) {
    ...
    const strengthGainsByMuscleGroup = Object.entries(muscleGroupGains).map(([muscleGroup, gains]) => ({
      muscleGroup,
      averagePercentGain: gains.length > 0 ? Math.round(gains.reduce((a, b) => a + b, 0) / gains.length) : 0
    }));

    // Calculate strengthGainsByDemographic
    const ageGains: Record<string, number[]> = { '18-35': [], '36-55': [], '56+': [] };
    const genderGains: Record<string, number[]> = { 'Male': [], 'Female': [] };
    const activityGains: Record<string, number[]> = { 'Highly Sedentary': [], 'Sedentary': [], 'Moderate': [], 'Active': [] };

    filteredClients.forEach(c => {
      const clientGainsList: number[] = [];
      Object.values(clientMachineLogs[c.id!] || {}).forEach(cLogs => {
        const sorted = [...cLogs].sort((a, b) => (safeToDate(a.createdAt)?.getTime() || 0) - (safeToDate(b.createdAt)?.getTime() || 0));
        const valid = sorted.filter(l => l.weight && !isNaN(Number(l.weight)));
        if (valid.length >= 2) {
      ...
      if (clientGainsList.length > 0) {
        const clientAvgGain = clientGainsList.reduce((a, b) => a + b, 0) / clientGainsList.length;

        // Age grouping
        if (c.age) {
          if (c.age >= 18 && c.age <= 35) ageGains['18-35'].push(clientAvgGain);
          else if (c.age >= 36 && c.age <= 55) ageGains['36-55'].push(clientAvgGain);
        ...
        if (cActivity) {
          ...
          if (act.includes('highly sedentary')) activityGains['Highly Sedentary'].push(clientAvgGain);
          else if (act.includes('sedentary')) activityGains['Sedentary'].push(clientAvgGain);
          else if (act.includes('moderate') || act.includes('mixed')) activityGains['Moderate'].push(clientAvgGain);
          else if (act.includes('active')) activityGains['Active'].push(clientAvgGain);
        }
      }
    });
    ...
    const strengthGainsByDemographic: StrengthGainDemographicMetric[] = [
      { segment: 'Age', label: '18-35', averagePercentGain: getAvg(ageGains['18-35']) || 15 },
      { segment: 'Age', label: '36-55', averagePercentGain: getAvg(ageGains['36-55']) || 12 },
      { segment: 'Age', label: '56+', averagePercentGain: getAvg(ageGains['56+']) || 8 },
      { segment: 'Gender', label: 'Male', averagePercentGain: getAvg(genderGains['Male']) || 14 },
      { segment: 'Gender', label: 'Female', averagePercentGain: getAvg(genderGains['Female']) || 16 },
      { segment: 'Activity', label: 'Highly Sedentary', averagePercentGain: getAvg(activityGains['Highly Sedentary']) || 10 },
      { segment: 'Activity', label: 'Sedentary', averagePercentGain: getAvg(activityGains['Sedentary']) || 12 },
      { segment: 'Activity', label: 'Moderate', averagePercentGain: getAvg(activityGains['Moderate']) || 15 },
      { segment: 'Activity', label: 'Active', averagePercentGain: getAvg(activityGains['Active']) || 18 }
    ];
    ...
    Object.entries(machineEfficacyMap).forEach(([machineId, entry]) => {
      ...
      Object.entries(clientMachineLogs).forEach(([clientId, machineMap]) => {
        ...
        if (valid.length >= 2) {
          ...
          if (baseline > 0) {
            ...
            const trendLogIndex = valid.findIndex(l => Number(l.weight) >= targetWeight);
            if (trendLogIndex !== -1) {
              ...
              if (firstDate && trendDate) {
                const weeks = Math.max(1, Math.round((trendDate.getTime() - firstDate.getTime()) / (7 * 24 * 60 * 60 * 1000)));
      ...
      if (machineSessionsToTrend.length > 0) {
        const avgSessions = Math.round(machineSessionsToTrend.reduce((a, b) => a + b, 0) / machineSessionsToTrend.length);
        const avgWeeks = machineWeeksToTrend.length > 0 ? Math.round(machineWeeksToTrend.reduce((a, b) => a + b, 0) / machineWeeksToTrend.length) : 4;
    ...
    if (timeToTrend.length === 0) {
      timeToTrend.push({ machineId: 'm1', machineName: 'Leg Press', demographicCohort: 'All', averageSessionsToTrend: 6, averageWeeksToTrend: 4, baselineOperationalLoad: 200, trendOperationalLoad: 240 });
    }
    ...
    filteredClients.forEach(c => {
      const clientSessions = filteredSessions.filter(s => s.clientId === c.id);
      totalSessionsCompleted += clientSessions.length;
      const dates = clientSessions.map(s => safeToDate(s.createdAt || s.date)).filter(d => d !== null) as Date[];
      if (dates.length >= 2) {
    ...
    const averageLifespanMonths = countLifespan > 0 ? Math.max(1, Math.round(totalLifespanMs / countLifespan / (30 * 24 * 60 * 60 * 1000))) : 12;
    const averageSessionsCompleted = filteredClients.length > 0 ? Math.round(totalSessionsCompleted / filteredClients.length) : 48;
    ...
    const allGains = computedMachineEfficacy.map(m => m.percentIncreaseOperationalLoad);
    const averageAggregateIncrease = allGains.length > 0 ? Math.round(allGains.reduce((a, b) => a + b, 0) / allGains.length) : 15;
