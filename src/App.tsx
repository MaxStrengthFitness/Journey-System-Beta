    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        try {
          ...
          setTrainers(
            trainersSnap.docs.map((d) => ({ id: d.id, ...d.data() }) as Trainer),
          );
          ...
          try {
            handleFirestoreError(error, OperationType.GET, "auth_initialization");
          } catch (thrownErr) {
            console.error("Auth initialization failed with formatted error:", thrownErr);
          }
        }
      } else {
        setAuthTrainer(null);
...
function AppContent({
  ...
  const startUnassignedSession = async () => {
    ...
    const activeStudioIdForSession =
      activeStudioId || authTrainer.primaryHomeStudioId;
  ...
  const handleClientSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { heightFeet, heightInches, ...restFinalData } =
  ...
  const updateClientSessions = async (
    clientId: string,
    current: number,
    delta: number,
  ) => {
