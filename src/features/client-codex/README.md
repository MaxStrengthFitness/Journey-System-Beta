...
| `pages/*.tsx` | One adapter per page. In the shell phase each hosts the long scroll's sections, moved unchanged into `pages/legacy-blocks.tsx`; each area replaces its adapter's contents in its own phase |
...
- **A door to a card scrolls it under the sticky bar** (`scroll-margin-top` from `--psub-stuck-h`); a page change with no card puts the page's top under the bar only if the bar is stuck, and never yanks the header away. Notes' own anchors (`notes-*`, `note-{id}`) are Notes' to interpret. Everything in the layout effect is guarded — `scrollIntoView` is feature-detected, and nothing may throw.
