...
1. **The ten-second read.** Today's tiles — booked, done, not completed, **never logged** (the loud one: past its slot, nothing marked; tap it for the list to chase), on the floor now — and the **Needs you** strip, which counts every action waiting on the page and jumps to it.
...
| Panel | Source | Rule |
| --- | --- | --- |
| Today | the week's schedule (`../changes/useWeekSchedule`, live, cancellations included) | `today.ts`: live bookings = the day's less the cancelled; never logged = a slot finished five minutes ago with nothing marked (`floor.ts`) |
...
| Team this week | the last 14 days of sessions; today's schedule | `team.ts`: completed since Monday, clients, hours, unlogged today — alphabetical, never ranked |
