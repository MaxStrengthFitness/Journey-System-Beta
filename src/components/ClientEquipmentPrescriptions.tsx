...
function MachineCard({
  ...
  const machineLogs = [...(allLogs || [])]
    .filter(
      (l: any) =>
        l.machineId === machine.id &&
        l.weight !== undefined &&
        l.weight !== "" &&
        !isNaN(Number(l.weight)),
    )
    .sort((a, b) => {
  ...
  const startingWeightDisplay =
    clientSetting?.startingWeight !== undefined
      ? clientSetting.startingWeight
      : "";
  const currentWeightDisplay =
    clientSetting?.currentWeight !== undefined
      ? clientSetting.currentWeight
      : "";
  ...
  const colors = {
    ...
    header:
      "bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/50",
  ...
  const [draftStartingWeight, setDraftStartingWeight] = useState(
    startingWeightDisplay?.toString() || "",
  );
  const [draftCurrentWeight, setDraftCurrentWeight] = useState(
    currentWeightDisplay?.toString() || "",
  );
  ...
  const handleSaveInit = async () => {
    if (!authTrainer) {
      toastError("Trainer session required.");
      return;
    }
    setIsSaving(true);
    try {
      ...
      await setDoc(
        docRef,
        {
          ...
          startingWeight: draftStartingWeight
            ? Number(draftStartingWeight)
            : null,
      ...
      toastSuccess("Initial setup saved successfully.");
    } catch (err) {
      console.error(err);
      toastError("Failed to save initial setup.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSaveWeight = async () => {
    ...
    if (
      draftStartingWeight === startingWeightDisplay?.toString() &&
      draftCurrentWeight === currentWeightDisplay?.toString()
    ) {
    ...
    try {
      ...
      await setDoc(
        docRef,
        {
          ...
          machineId: machine.id,
          startingWeight:
            draftStartingWeight !== "" ? Number(draftStartingWeight) : null,
          currentWeight:
            draftCurrentWeight !== "" ? Number(draftCurrentWeight) : null,
          updatedAt: new Date(),
        },
        { merge: true },
      );
      ...
      setWeightDialogOpen(false);
      toastSuccess("Weight configuration updated successfully.");
    } catch (err) {
      console.error(err);
      toastError("Failed to save weight.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSaveSettings = async () => {
    ...
    if (hasData && !reason.trim()) {
      toastError("Please provide a reason for the setting adjustment.");
      return;
    }
    setIsSaving(true);
    try {
      ...
      toastSuccess("Machine settings updated successfully.");
    } catch (err) {
      console.error(err);
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      toastSuccess("Note appended successfully.");
    } catch (err) {
      console.error(err);
        ...
        <div className="flex items-center gap-2 shrink-0">
          {isExpandedCompact && (
            <Button
              ...
              className="h-11 min-w-11 px-3.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300 font-black uppercase tracking-widest text-[11px]"
            >
              Minimize
            </Button>
          )}
      ...
      >
        {!hasData ? (
          ...
          <div className="flex items-start justify-between">
            <div
              className={cn(
                "border-l-4 border border-slate-200 dark:border-slate-800 rounded-md p-2 px-3 bg-white dark:bg-slate-950",
            ...
            <div className="text-right">
              ...
              <div className="flex flex-col items-end gap-1">
                {orderMachineSettings(
                  ...
                  standardSettings,
                  options,
                ).map(([shortKey, v, originalKey]) => {
                  const isStandard = v === standardSettings[originalKey];
                  return (
                    <span
                      key={originalKey}
                      className={cn(
                        "text-[11px] uppercase font-bold tracking-widest px-1.5 py-0.5 rounded-md flex items-center justify-end min-w-8",
                        isStandard
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <Dialog
                ...
                onOpenChange={(open) => {
                  ...
                  if (open) {
                    setDraftStartingWeight(
                      startingWeightDisplay?.toString() || "",
                    );
                    setDraftCurrentWeight(
                      currentWeightDisplay?.toString() || "",
                    );
                  }
                }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setDraftStartingWeight(e.target.value)
                          }
                          className={cn(
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setDraftCurrentWeight(e.target.value)
                          }
                          className={cn(
                            "h-14 text-xl text-center font-black rounded-2xl bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 transition-all dark:text-white",
                            "focus-visible:ring-indigo-500",
                ...
                onOpenChange={(open) => {
                  ...
                  if (open) {
                    ...
                    options.forEach((opt: string) => {
                      ...
                      const foundKey = Object.keys(currentSettings).find(
                        (k) => {
                      ...
                      if (foundKey) {
                        normalizedCurrentSettings[opt] = String(
                          currentSettings[foundKey],
                        );
                      } else {
                      ...
                      if (
                        lowerOpt === "gap" &&
                        (normalizedCurrentSettings[opt] === undefined ||
                          normalizedCurrentSettings[opt] === null ||
                          normalizedCurrentSettings[opt] === "")
                      ) {
                        const stdGapKey = standardSettings
                          ? Object.keys(standardSettings).find(
                              (k) => k.toLowerCase() === "gap",
                            )
                          : undefined;
                        if (
                          stdGapKey &&
                          standardSettings &&
                          standardSettings[stdGapKey] !== undefined &&
                          standardSettings[stdGapKey] !== null &&
                          standardSettings[stdGapKey] !== ""
                        ) {
                          normalizedCurrentSettings[opt] = String(
                            standardSettings[stdGapKey],
                          );
...
export function ClientEquipmentPrescriptions({
  ...
  const isMachinePerformed = (m: any) => {
    ...
    const hasWeight =
      cPreset &&
      ((cPreset.startingWeight !== undefined &&
        cPreset.startingWeight !== null &&
        cPreset.startingWeight !== "") ||
        (cPreset.currentWeight !== undefined &&
          cPreset.currentWeight !== null &&
          cPreset.currentWeight !== ""));
    const hasSettings =
      cPreset && cPreset.settings && Object.keys(cPreset.settings).length > 0;
  ...
  const machinesWithStartingWeightCount = sortedMachines.filter(
    (machine: any) => {
  ...
  const pctStarting =
    sortedMachines.length > 0
      ? Math.round(
          (machinesWithStartingWeightCount / sortedMachines.length) * 100,
        )
      : 0;

  const clientFlags = client?.clinicalFlags || [];
  const machinesWithWarningsCount = sortedMachines.filter((machine: any) => {
    return CLINICAL_FLAGS_MATRIX.some(
      (flag: any) =>
        clientFlags.includes(flag.id) &&
        flag.protocolHandling?.some((ph: any) =>
          ph.affectedMachineIds?.includes(machine.id),
        ),
    );
  }).length;
  ...
  const handleMassApplyStandards = async () => {
    ...
    try {
      const missingOverride = sortedMachines.filter((m: any) => {
        ...
        return (
          !settingObj?.settings || Object.keys(settingObj.settings).length === 0
        );
      });

      if (missingOverride.length === 0) {
        toastError("No machines are missing a per-client override setup.");
      ...
      setMassApplyOpen(false);
      toastSuccess("Standard settings mass-applied successfully.");
    } catch (err) {
      console.error(err);
      toastError("Failed to mass-apply standards.");
    } finally {
            ...
            <DialogContent className="max-w-md rounded-[32px] p-6 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
              ...
              <div className="space-y-4">
                <p className="text-xs text-slate-600 dark:text-slate-400">
                  Are you sure you want to mass-apply standard settings from the
                  current studio standard templates? This will initialize any
                  unconfigured machines. Existing personalized configs will NOT
                  be modified.
                </p>
      ...
      {viewMode === "compact" ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {sortedMachines.map((machine: any) => {
                ...
                <div key={machine.id} className="col-span-1">
                  <MachineCard
                    ...
                    clientExperienceLevel={
                      ...
                      client?.infoForm?.experienceLevel
                    }
                    allLogs={allLogs}
            ...
            const gapKey = Object.keys(currentSettings).find(
              (k) => k.toLowerCase().trim() === "gap",
            );
            ...
            const repsVal = lastLog
              ? lastLog.repsLeft !== undefined &&
                lastLog.repsRight !== undefined
            ...
            return (
              <div
                ...
                className={cn(
                  "h-27.5 rounded-xl bg-bg-l-card dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 flex flex-col justify-between cursor-pointer hover:border-[#F06C22]/50 transition-all select-none relative group border-l-4",
                  baseColorStyles.border,
                )}
              >
                <div className="flex items-start justify-between gap-2 min-w-0">
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    {isRomSet && (
                      <Badge
                        variant="outline"
                        className="text-[11px] font-bold uppercase py-0 px-1.5 h-5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 rounded-md"
                      >
                        ✓ ROM
                      </Badge>
                    )}
