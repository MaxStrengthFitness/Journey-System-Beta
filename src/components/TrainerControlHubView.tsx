        ...
        <div className="w-full lg:w-64 shrink-0 flex flex-col gap-2">
          {[
            ...
            return (
              ...
              >
                <Icon className={cn("w-5 h-5", activeTab === tab.id ? "text-cta" : "opacity-50")} />
                {tab.label}
              </button>
            )
          })}
              ...
              <CardHeader className="bg-background pb-8 border-b border-border">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-2xl font-black text-foreground italic tracking-tight">Team Management</CardTitle>
                      <CardDescription className="text-muted-foreground font-medium uppercase text-[11px] tracking-widest">Manage individual Schedule Sync URLs.</CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {isAdmin && onReorderTrainers && (
                      <Button 
                        ...
                        className="rounded-xl border-border text-muted-foreground bg-card hover:text-foreground hover:bg-muted text-foreground h-10 px-4 font-black uppercase text-[11px] tracking-widest gap-2"
                      >
                  ...
                  {visibleTrainers.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      ...
                      <div className="space-y-4">
                        <Label className="text-xs font-black uppercase tracking-widest text-muted-foreground">Roster</Label>
                        ...
                        <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                          {filteredTrainers.length === 0 ? (
                            <p className="text-center py-6 text-muted-foreground font-medium italic text-xs">No matching trainers.</p>
                          ) : (
                                ...
                                >
                                  <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      <p className="text-xs font-black text-foreground uppercase italic truncate">{t.fullName}</p>
                        ...
                        {currentSelectedTrainer ? (
                          <div className="p-6 bg-background border border-border rounded-[24px] space-y-6">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border">
                              <div className="flex items-center gap-4">
                                <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl italic shrink-0", checkIsOwner(currentSelectedTrainer) ? 'bg-amber/10  text-amber border border-amber/30 ' : 'bg-card text-muted-foreground  border border-border ')}>
                                  {currentSelectedTrainer.initials}
                                </div>
                                <div>
                                  <div className="flex flex-wrap items-center gap-2">
                                    <h3 className="text-lg font-black text-foreground uppercase italic leading-none">{currentSelectedTrainer.fullName}</h3>
                                    {checkIsOwner(currentSelectedTrainer) && (
                                      <span className="bg-amber/10 text-amber border border-amber/30 px-1.5 py-0.5 rounded text-[11px] font-black uppercase">Owner</span>
                                    )}
                                  </div>
                              ...
                              <div className="flex items-center justify-between p-3 px-4 bg-card rounded-xl border border-border">
                                <Label className="text-xs font-bold text-muted-foreground cursor-pointer mr-3">Show on Hub Calendar</Label>
                                <Switch 
                            ...
                            <div className="flex flex-col gap-2 p-4 bg-card rounded-2xl border border-border">
                              <Label className="text-[11px] font-black uppercase text-muted-foreground tracking-widest leading-none">Primary Home Studio</Label>
                            ...
                            <div className="flex flex-col gap-3 p-4 bg-card rounded-2xl border border-border">
                              <Label className="text-[11px] font-black uppercase text-muted-foreground tracking-widest leading-none">Accessible Studios (Cross-Training)</Label>
                              <div className="flex flex-wrap gap-2">
                                {(currentSelectedTrainer.accessibleStudioIds || []).length === 0 ? (
                                  <span className="text-xs text-muted-foreground italic">No secondary locations assigned</span>
                                ) : (
                                  (currentSelectedTrainer.accessibleStudioIds || []).map(studioId => {
                                    const s = studios.find(st => st.id === studioId);
                                    if (!s) return null;
                                    return (
                                      <div key={studioId} className="flex items-center gap-1.5 pl-3 pr-1 py-1.5 rounded-xl bg-cta/10 border border-cta/20 text-brand font-bold text-xs">
                                        <span>{s.name}</span>
                                        <button
                                          type="button"
                                ...
                                {editingIcalId === currentSelectedTrainer.id ? (
                                  <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex items-center justify-end gap-1.5">
                                      <Button variant="ghost" size="sm" onClick={() => setEditingIcalId(null)} className="h-8 px-3 font-bold uppercase text-[11px] rounded-lg tracking-widest text-muted-foreground">Cancel</Button>
                                      <Button 
                                        size="sm"
                                        onClick={() => {
                                          if (!currentSelectedTrainer.id) return;
                                  ...
                                  <div className="flex items-center justify-between gap-2 overflow-hidden group/link bg-card p-3 rounded-xl border border-border">
                                    {currentSelectedTrainer.mindbody_ical_url ? (
                                      <>
                                        ...
                                        <span className="text-[11px] text-muted-foreground font-medium truncate flex-1">{currentSelectedTrainer.mindbody_ical_url}</span>
                                        <Button 
                                          variant="ghost" 
                                          size="sm" 
                                          onClick={() => {
                                      ...
                                      <>
                                        <span className="text-[11px] text-muted-foreground font-medium italic select-none">No feed configured</span>
                                        <Button 
                                          variant="outline" 
                                          size="sm" 
                                          onClick={() => {
                                          ...
                                          className="h-8 border-border bg-card text-muted-foreground hover:text-foreground hover:bg-muted hover:text-foreground rounded-lg px-4 font-black uppercase text-[11px] tracking-widest"
                                        >
                                          Add Link
                                        </Button>
                                      </>
                          ...
                          <div className="h-full flex flex-col items-center justify-center py-20 px-6 border border-dashed border-border rounded-[24px] text-center bg-muted/50">
                            ...
                            <h4 className="font-bold text-foreground">No Trainer Selected</h4>
                            <p className="text-xs text-muted-foreground mt-1 max-w-xs">Select a trainer from the list to synchronize their scheduling feeds and control accessible home studios.</p>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <CardHeader className="bg-background pb-6 border-b border-border">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4 flex-wrap">
                            ...
                            <div>
                              <CardTitle className="text-2xl font-black text-foreground italic tracking-tight">Live Floor Status</CardTitle>
                              <CardDescription className="text-muted-foreground font-medium uppercase text-[11px] tracking-widest">Active sessions and concurrency monitoring.</CardDescription>
                            </div>
                          </div>
                          <div className="px-4 py-2 bg-card rounded-xl border border-border shrink-0">
                            <span className="text-[11px] font-black uppercase text-green tracking-widest animate-pulse">● System Live</span>
                          </div>
                        </div>
                      </CardHeader>
                      <CardContent className="p-8 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="p-5 bg-green/10 rounded-2xl border border-green/30">
                            <p className="text-[11px] font-black uppercase tracking-widest text-green mb-1 leading-none">Total Active</p>
                            <p className="text-3xl font-black text-foreground italic mt-2 leading-none">{sessions.filter(s => s.status === 'In-Progress' && isSessionValid(s)).length}</p>
                          </div>
                          <div className="p-5 bg-amber/10 rounded-2xl border border-amber/30">
                            <p className="text-[11px] font-black uppercase tracking-widest text-amber mb-1 leading-none">Stale Sessions</p>
                            <p className="text-3xl font-black text-amber italic mt-2 leading-none">{sessions.filter(s => s.status === 'In-Progress' && !isSessionValid(s)).length}</p>
                          </div>
                          <div className="p-5 bg-cta/10 rounded-2xl border border-cta/20">
                            <p className="text-[11px] font-black uppercase tracking-widest text-cta mb-1 leading-none">Current Sync</p>
                        ...
                        <div className="space-y-3">
                          <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground px-1">Active Floor Feed</h3>
                            ...
                            {sessions.filter(s => s.status === 'In-Progress').map(session => {
                              ...
                              return (
                                <div key={session.id} className={cn(
                                  ...
                                  isValid ? "bg-card border-border" : "bg-background border-amber/20 opacity-70 grayscale"
                                )}>
                                  <div className="flex items-center gap-3 min-w-0 flex-1">
                                    <div className={cn(
                                      "w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs border shrink-0",
                                      isValid ? "bg-muted text-cta border-border" : "bg-amber/10 text-amber border-amber/30"
                                    )}>
                                      {client ? `${client.firstName[0]}${client.lastName[0]}` : "UN"}
                                    </div>
                                    <div className="min-w-0 flex-1">
                                      <div className="flex items-center gap-1.5 flex-wrap">
                                        <h4 className="font-bold text-foreground text-xs truncate max-w-[150px]">{client ? `${client.firstName} ${client.lastName}` : "Unassigned Session"}</h4>
                                        {!isValid && <span className="bg-amber/10 text-amber border border-amber/30 px-1 rounded-[4px] text-[7px] font-black uppercase">Abandoned</span>}
                                      </div>
                    ...
                    <Card className="border border-border bg-card shadow-sm rounded-[32px] overflow-hidden">
                      <CardHeader className="bg-background pb-6 border-b border-border">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <CardTitle className="text-2xl font-black text-foreground italic tracking-tight">Studio Configuration</CardTitle>
                            <CardDescription className="text-muted-foreground font-medium uppercase text-[11px] tracking-widest">Manage franchise locations.</CardDescription>
                          </div>
                        </div>
                      </CardHeader>
                        ...
                        <div className="space-y-4">
                          {studios.length > 0 ? (
                            <div className="border border-border rounded-2xl overflow-hidden divide-y divide-border/60 bg-card">
                              {studios.map(studio => (
                                <div key={studio.id} className="flex items-center justify-between p-4 px-5 hover:bg-muted/50 transition-colors">
                                  <span className="text-xs font-black uppercase text-foreground tracking-wider font-mono">{studio.name}</span>
                                  <Button
                                    variant="ghost"
                                    size="sm"
                      ...
                      <Card className="border border-border bg-card shadow-sm rounded-[32px] overflow-hidden">
                        <CardHeader className="bg-background pb-6 border-b border-border">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <CardTitle className="text-2xl font-black text-foreground italic tracking-tight">Hub Announcements</CardTitle>
                              <CardDescription className="text-muted-foreground font-medium uppercase text-[11px] tracking-widest">Share informative insights with your team.</CardDescription>
                            </div>
                          </div>
                        </CardHeader>
                        <CardContent className="p-8 space-y-6">
                          ...
                          <div className="p-5 bg-muted/50 rounded-[24px] border border-border space-y-4">
                            <div className="flex items-center gap-2">
                              ...
                              <h3 className="text-xs font-black text-foreground uppercase tracking-wider italic leading-none">Draft New Message</h3>
                            </div>
                            
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              <div className="space-y-1">
                                <Label className="text-[11px] font-bold uppercase text-muted-foreground">Headline</Label>
                                <Input 
                              ...
                              <div className="space-y-1">
                                <Label className="text-[11px] font-bold uppercase text-muted-foreground">Audience</Label>
                                <Select 
                            ...
                            <div className="space-y-1">
                              <Label className="text-[11px] font-bold uppercase text-muted-foreground">Short Snippet (Quick Header View)</Label>
                              <Input 
                            ...
                            <div className="space-y-1">
                              <Label className="text-[11px] font-bold uppercase text-muted-foreground">Full Details (Expanded View)</Label>
                              <Textarea 
                                ...
                                className="bg-card border-border text-foreground text-xs min-h-[80px] rounded-xl font-medium"
                              />
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-3 border-t border-border">
                              <div className="flex flex-wrap items-center gap-4">
                                <div className="flex items-center gap-2">
                                  <Label className="text-[11px] font-bold uppercase text-muted-foreground mt-0.5">Priority:</Label>
                                  <Select 
                                ...
                                <div className="flex items-center gap-2">
                                  <Label className="text-[11px] font-bold uppercase text-muted-foreground mt-0.5">Type:</Label>
                                  <Select 
                                ...
                                <div className="flex items-center gap-2">
                                  <Label className="text-[11px] font-bold uppercase text-muted-foreground mt-0.5">Lifespan:</Label>
                                  <Select 
                                    value={lifespan} 
                          ...
                          <div className="space-y-3.5">
                            <h3 className="text-[11px] font-black uppercase tracking-widest text-muted-foreground px-1">Active Messages</h3>
                            {announcements.length === 0 ? (
                              ...
                              <div className="space-y-3">
                                {announcements.map(a => (
                                  <div key={a.id} className="p-4 bg-card border border-border rounded-2xl flex items-center justify-between group">
                                    <div className="flex items-start gap-3 min-w-0 flex-1">
                                      <div className={cn(
                                        "w-9 h-9 rounded-xl flex items-center justify-center border shrink-0 mt-0.5",
                                        a.priority === 'high' ? "bg-red/10 border-red/30 text-red" :
                                        a.priority === 'medium' ? "bg-amber/10 border-amber/30 text-amber" :
                                      ...
                                      <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-1.5 flex-wrap">
                                          <h4 className="font-black text-foreground italic text-xs truncate max-w-[155px]">{a.title}</h4>
                                        ...
                                        <p className="text-[11px] text-muted-foreground mt-1 truncate">{a.shortContent}</p>
                  ...
                  <div className="border border-border bg-card shadow-sm rounded-[32px] overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-2xl font-black text-foreground italic tracking-tight">Hardware & Equipment Configuration</h3>
                          <p className="text-muted-foreground font-medium uppercase text-[11px] tracking-widest">Map, test, and calibrate live floor biometric machines.</p>
                        </div>
                      </div>
                      <div className="p-2 bg-muted rounded-xl">
                        <ChevronDown className={cn("w-5 h-5 text-muted-foreground transition-transform duration-200", isEquipmentExpanded && "rotate-180")} />
                      </div>
                    </button>
                    {isEquipmentExpanded && (
                ...
                <div className="flex items-center justify-center p-6 min-h-[400px] animate-fade-in">
                  <Card className="border border-border bg-card shadow-sm rounded-[32px] p-8 max-w-md w-full text-center space-y-6">
                    ...
                    <div className="space-y-2">
                      <h3 className="text-xl font-black text-foreground uppercase italic tracking-tight">Access Restricted</h3>
                ...
                <Card className="border border-border bg-card shadow-sm rounded-[32px] overflow-hidden">
                  <CardHeader className="bg-background pb-8 border-b border-border">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <CardTitle className="text-2xl font-black text-foreground italic tracking-tight">Legacy Data Ingestion Pipeline</CardTitle>
                        <CardDescription className="text-muted-foreground font-medium uppercase text-[11px] tracking-widest">Import historical client logs via CSV to populate the demographic engine.</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="p-8 space-y-8">
                    ...
                    <div className="space-y-4">
                      <Label htmlFor="legacy-upload" className="text-[11px] font-black uppercase tracking-widest text-muted-foreground px-1">Historical Workout Data (CSV)</Label>
                        ...
                        <div className="w-full h-44 border-2 border-dashed border-border bg-background rounded-3xl flex flex-col items-center justify-center p-6 text-center transition-all duration-200 group-hover:bg-muted/70 group-hover:border-cta">
                          {isLegacyImporting ? (
                            <div className="flex flex-col items-center gap-3">
                              ...
                              <span className="text-sm font-black uppercase italic tracking-wider text-cta">Processing Legacy Data...</span>
                            </div>
                          ) : (
                            <div className="flex flex-col items-center space-y-3">
                              ...
                              <div className="space-y-1">
                                <p className="text-xs font-black text-foreground uppercase tracking-wider">Drag & drop or click to upload CSV</p>
                                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Supports legacy FileMaker schemas & metrics</p>
                              </div>
                            </div>
                          )}
                        </div>
                    ...
                    {legacyStats && (
                      <div className="p-6 bg-green/10 border border-green/30 rounded-2xl flex items-start gap-4">
                        ...
                        <div>
                          <p className="font-black text-green uppercase italic tracking-wider text-sm">Import Success</p>
                          <div className="text-muted-foreground font-bold uppercase tracking-widest grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-2 mt-3 text-[11px]">
                            <p>Clients: <span className="font-mono text-xs text-foreground font-black">{legacyStats.clients}</span></p>
                            <p>Sessions: <span className="font-mono text-xs text-foreground font-black">{legacyStats.sessions}</span></p>
                            <p>Logs: <span className="font-mono text-xs text-foreground font-black">{legacyStats.logs}</span></p>
                            <p>Skipped: <span className="font-mono text-xs text-foreground font-black">{legacyStats.failed}</span></p>
                          </div>
                        </div>
                      </div>
                    )}
      ...
      {trainerToDelete && (
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            ...
            <div>
              <h3 className="text-xl font-black text-foreground italic tracking-tighter uppercase">Delete Trainer?</h3>
              <p className="text-muted-foreground mt-2 text-sm leading-relaxed">
                Are you sure you want to remove <strong className="text-foreground">{trainerToDelete?.fullName}</strong>? This action cannot be undone.
              </p>
            </div>
            <div className="flex gap-3 w-full">
              <Button onClick={() => setTrainerToDelete(null)} variant="outline" className="flex-1 rounded-xl h-12 bg-card border-border text-foreground hover:bg-muted text-foreground font-bold uppercase tracking-widest text-[11px]">
                Cancel
              </Button>
              <Button onClick={handleDeleteTrainer} variant="destructive" className="flex-1 rounded-xl h-12 bg-red hover:bg-red/80 text-foreground font-black uppercase tracking-widest shadow-sm dark:shadow-none border-none text-[11px]">
                Confirm
              </Button>
            </div>
          </motion.div>
