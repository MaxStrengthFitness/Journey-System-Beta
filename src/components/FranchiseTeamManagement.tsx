  ...
  const handleToggleVisibility = async (id: string, currentValue: boolean) => {
    try {
      if (!id)
        throw new DocumentIdMissingError("trainers", OperationType.UPDATE);
      await updateDoc(doc(db, "trainers", id), {
        isVisibleOnCalendar: !currentValue,
      });
    } catch (e) {
      console.error(e);
    }
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p
                          className={cn(
                            "text-[11px] font-bold uppercase tracking-widest truncate mt-0.5",
                            getRoleColor(t.role).split(" ")[0],
                          )}
                        >
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-[24px] space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
                <div className="flex items-center gap-4">
                  <div
                    className={cn(
                      ...
                      getRoleColor(currentSelectedTrainer.role),
                    )}
                  >
                  ...
                  <div>
                    ...
                    <p
                      className={cn(
                        "text-[11px] font-bold uppercase tracking-widest leading-none mt-2",
                        getRoleColor(currentSelectedTrainer.role).split(" ")[0],
                      )}
                    >
                ...
                <div className="flex items-center justify-between p-3 px-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  <Switch
                    checked={
                      currentSelectedTrainer.isVisibleOnCalendar !== false
                    }
                    onCheckedChange={() => {
                      if (!currentSelectedTrainer.id) return;
                      handleToggleVisibility(
                        currentSelectedTrainer.id,
                        currentSelectedTrainer.isVisibleOnCalendar ?? true,
                      );
                    }}
              ...
              <div className="flex flex-col gap-2 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
                ...
                <Select
                  value={
                    currentSelectedTrainer.primaryHomeStudioId || "unassigned"
                  }
                  onValueChange={(val) => {
                    if (!currentSelectedTrainer.id) return;
              ...
              <div className="flex flex-col gap-3 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  0 ? (
                    ...
                    (currentSelectedTrainer.accessibleStudioIds || []).map(
                      (studioId) => {
                        ...
                        return (
                          <div
                            key={studioId}
                            className="flex items-center gap-1.5 pl-3 pr-1 py-1.5 rounded-xl bg-sky-50 dark:bg-sky-950/40 border border-sky-100 dark:border-sky-900/30 text-sky-900 dark:text-sky-300 font-bold text-xs"
                          >
                            <span>{s.name}</span>
                            <button
                              type="button"
                ...
                {editingIcalId === currentSelectedTrainer.id ? (
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-mono text-slate-600 dark:text-slate-400 break-all pr-4">
                      {currentSelectedTrainer.thirdPartyCalendarUrl ||
            ...
            <div className="h-full min-h-100 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 border-dashed rounded-[32px] flex items-center justify-center p-8">
              <div className="text-center space-y-3 max-w-sm">
                ...
                <p className="text-[11px] font-medium text-slate-500 uppercase tracking-widest">
                  Choose a trainer from the roster list to view and manage their
                  schedule and studio access.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-[24px] max-w-sm w-full border border-slate-200 dark:border-slate-800 shadow-2xl">
            ...
            <p className="text-xs text-slate-500 font-medium mb-6">
              ...
              {trainerToDelete.fullName}? This cannot be undone.
            </p>
            <div className="flex gap-3 justify-end">
              ...
              <Button
                ...
                className="h-10 text-xs font-bold uppercase bg-rose-500 hover:bg-rose-600 text-white"
              >
                Delete Permanently
              </Button>
            </div>
