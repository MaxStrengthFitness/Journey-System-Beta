...
export function AdminDashboardView({ authTrainer, studios, isAdmin }: Props) {
  const [activeTab, setActiveTab] = useState<'metrics' | 'users' | 'studios' | 'announcements' | 'bugs'>('metrics');

  const tabs = [
    { id: 'metrics', label: 'Overview', icon: <Activity className="w-4 h-4" /> },
    { id: 'users', label: 'Staff & Roles', icon: <Users className="w-4 h-4" /> },
    { id: 'studios', label: 'Franchises', icon: <Building2 className="w-4 h-4" /> },
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 animate-fade-in text-slate-900 dark:text-slate-100">
      ...
      <div className="mt-6">
        {activeTab === 'metrics' && <AdminMetricsDashboard onManageStudios={() => setActiveTab('studios')} />}
        {activeTab === 'users' && <AdminUserDirectory studios={studios} />}
        {activeTab === 'studios' && (
          <AdminStudioManager 
        ...
        {activeTab === 'announcements' && <AdminHubAnnouncements studios={studios} authTrainer={authTrainer} />}
