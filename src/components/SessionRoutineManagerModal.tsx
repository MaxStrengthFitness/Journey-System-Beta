...
export function SessionRoutineManagerModal({ isOpen, onOpenChange, currentMachineIds, machines, onSave }: Props) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] sm:max-w-[95vw] md:max-w-[95vw] w-full bg-[#0B1724] border-slate-700 text-white p-0 overflow-hidden shadow-2xl flex flex-col h-[70vh]">
        <DialogHeader className="p-4 md:p-6 bg-slate-900 border-b border-slate-800 shrink-0 relative z-20">
          <DialogTitle className="text-xl md:text-2xl font-black uppercase tracking-widest text-white">Edit Routine Sequence</DialogTitle>
          <DialogDescription className="text-slate-400 font-bold uppercase tracking-widest text-[10px] md:text-xs mt-1 md:mt-2">
                ...
                return (
                  <motion.div
                    ...
                    className={`
                      ...
                      ${isSelected 
                        ? 'bg-[#0A2E46] border-[#F06C22] shadow-[0_0_15px_rgba(240,108,34,0.15)] opacity-100 z-10' 
                        : 'bg-[#115E8D]/10 border-slate-800/50 opacity-50 hover:opacity-100 hover:bg-[#115E8D]/20 hover:border-slate-700 grayscale-[0.8] hover:grayscale-0'
                      }
                    `}
                  >
                      ...
                      <AnimatePresence mode="wait">
                        {isSelected ? (
                          ...
                          <motion.div 
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="w-8 h-8 rounded-full border border-slate-700 flex items-center justify-center group-hover:border-slate-500 transition-colors"
                          >
                            <span className="text-slate-700 font-black group-hover:text-slate-500 text-[10px] uppercase">+</span>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
                    ...
                    <div className={`flex-1 text-left flex flex-col justify-center ${isSelected ? 'pr-6 md:pr-8' : ''}`}>
                      <h3 className={`text-xs md:text-sm font-black uppercase tracking-widest leading-tight ${isSelected ? 'text-white' : 'text-slate-500 group-hover:text-slate-300'}`} title={machine.name}>
                        {getShorthand(machine.name || '')}
                      </h3>
                      <div className="flex items-center gap-2 mt-0.5">
                         <p className={`text-[8px] md:text-[9px] font-bold uppercase tracking-widest ${isSelected ? 'text-[#F06C22]' : 'text-slate-600'}`}>
                    ...
                    {isSelected && (
                      <div className="absolute right-1.5 md:right-3 flex flex-col gap-1 items-center justify-center">
                        <button 
                          ...
                          className="p-1 rounded bg-slate-800/80 hover:bg-[#F06C22] text-slate-300 hover:text-white disabled:opacity-30 transition-colors shadow-sm disabled:hover:bg-slate-800/80 disabled:cursor-not-allowed"
        ...
        <DialogFooter className="p-4 md:p-6 bg-slate-900 border-t border-slate-800 shrink-0 relative z-20 flex flex-row items-center justify-end gap-3">
          <Button 
            variant="outline" 
            onClick={() => onOpenChange(false)}
            className="border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-white tracking-widest uppercase font-black"
          >
            Cancel
          </Button>
          <Button 
