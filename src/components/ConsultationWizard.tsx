...
export function ConsultationWizard({
  ...
  useEffect(() => {
    if (routine.length === 0 && machines.length > 0) {
      // Find matches in the dynamic machines list, falling back to similar machine types
      const hasLegPress = machines.some(m => m.name === "Leg Press" || m.id === "leg-press");
      const hasDip = machines.some(m => m.name === "Seated Dip" || m.id === "seated-dip");
      const hasChestPress = machines.some(m => m.name === "Chest Press" || m.id === "chest-press");
      const hasLumbar = machines.some(m => m.name === "Lumbar" || m.name === "Lumbar Extension" || m.id === "lumbar");

      const selectedLegPress = hasLegPress ? "Leg Press" : (machines.find(m => m.name.toLowerCase().includes("press") && m.name.toLowerCase().includes("leg"))?.name || machines[0]?.name || "Leg Press");
      
      const pushDefault = client.gender === "Female" ? (hasDip ? "Seated Dip" : "Chest Press") : (hasChestPress ? "Chest Press" : "Seated Dip");
      const selectedPush = machines.some(m => m.name === pushDefault) ? pushDefault : (machines.find(m => m.name.toLowerCase().includes("press") || m.name.toLowerCase().includes("dip"))?.name || machines[1]?.name || pushDefault);
      
      const selectedLumbar = hasLumbar ? (machines.find(m => m.name === "Lumbar" || m.name === "Lumbar Extension")?.name || "Lumbar") : (machines.find(m => m.name.toLowerCase().includes("lumbar") || m.name.toLowerCase().includes("back"))?.name || machines[2]?.name || "Lumbar");
  ...
  const handleComplete = async () => {
    if (!canProceed) {
      toastError("Please check all consideration boxes before proceeding.");
      return;
    }
    ...
    try {
      ...
      const customMachineIds = routine
        .map((name) => {
  ...
  const addMachine = () => {
    const defaultAvailable = machines.filter(
      (m) => !routine.includes(m.name),
    )[0];
    if (defaultAvailable) {
    ...
    <div className="flex flex-col bg-[#0A2E46] min-h-screen text-white">
      ...
      <div className="p-8 pt-12 mb-2 bg-linear-to-b from-black/20 to-transparent flex justify-between items-start sticky top-0 z-50 backdrop-blur-md">
        ...
        <Button
          ...
          size="icon"
          onClick={onCancel}
          className="text-[#94A3B8] hover:text-white hover:bg-white/10 shrink-0"
        >
          <X className="w-8 h-8" />
        </Button>
      </div>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-8 flex flex-col gap-6">
            <div className="space-y-3">
              <label className="text-[11px] font-bold uppercase tracking-widest text-[#94A3B8] ml-2">
                Occupation
              </label>
              <Textarea
                value={occupation}
                onChange={(e) => setOccupation(e.target.value)}
                className="bg-black/20 border-white/10 text-lg min-h-25 resize-none"
            ...
            <div className="space-y-3">
              <div className="bg-[#115E8D]/20 border border-[#115E8D] rounded-xl p-4">
                <p className="text-sm italic font-bold text-[#38BDF8]">
                  "With our therapeutic approach, we like to fix people rather
                  than break them."
                </p>
              </div>
              <label className="text-[11px] font-bold uppercase tracking-widest text-[#94A3B8] ml-2">
                Medical History & Orthopedic Concerns
              </label>
              <Textarea
                value={medical}
            ...
            <div className="space-y-3">
              <div className="bg-[#115E8D]/20 border border-[#115E8D] rounded-xl p-4">
                <p className="text-sm italic font-bold text-[#38BDF8]">
                  "This is a result-driven program. We help clients with [Goal]
                  every day."
                </p>
              </div>
              <label className="text-[11px] font-bold uppercase tracking-widest text-[#94A3B8] ml-2">
                Primary Goals (The Deep &quot;Why&quot;)
              </label>
              <Textarea
                value={goals}
          ...
          <div className="space-y-4 flex flex-col gap-2">
            ...
            >
              <div className="mt-1 shrink-0">
                <Checkbox
                  checked={cbGum}
                  className="w-8 h-8 rounded border-2 border-white/40 data-[state=checked]:bg-[#38BDF8] data-[state=checked]:border-[#38BDF8]"
                />
              </div>
              <div>
                <h3 className="text-xl font-black uppercase tracking-tighter text-white mb-2">
                  Nothing loose in mouth
                </h3>
                <p className="text-sm text-[#94A3B8] leading-relaxed">
                  "Gum, cough drops, etc., become a choking hazard. Please
                  discard before training."
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-[#94A3B8] leading-relaxed">
                  "Avoid holding your breath at all costs—your BP goes up 2-4
                  times its normal rate. Breathe more than you think you have
                  to."
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-[#94A3B8] leading-relaxed">
                  "Don't be concerned with a 'good workout' today. We're
                  focusing on concepts and form."
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4 flex flex-col gap-4 mt-8">
            {routine.map((machineName, idx) => {
              ...
              const machineDef = machines.find(
                (m) => m.name === machineName || m.fullName === machineName,
              );
              const tip =
                machineDef?.settings || "Setup tailored to individual client.";
              const category =
                MACHINE_DICTIONARY[machineName]?.category || "General";

              return (
                <Card
                  key={idx}
                  className="bg-[#F8FAFC] border-none shadow-2xl overflow-hidden rounded-[24px]"
                >
                      ...
                      <div className="flex justify-between items-start mb-2 gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          >
                            ...
                            <SelectContent className="max-h-75">
                              {machines.map((m) => {
                                const cat =
                                  MACHINE_DICTIONARY[m.name]?.category ||
                                  "General";
                                return (
                                  <SelectItem
                                    key={m.id}
                          ...
                          <div className="mt-1 mb-3">
                            <Badge
                              variant="secondary"
                              className="text-[11px] uppercase font-bold tracking-widest bg-slate-200 text-slate-600 rounded-md"
                            >
                              {category}
                            </Badge>
                          </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-right flex flex-col items-end pl-4 border-l border-slate-200 shrink-0">
                            <span className="text-[11px] font-bold uppercase tracking-widest text-[#68717A] mb-2">
                              Starting Wt
                            </span>
