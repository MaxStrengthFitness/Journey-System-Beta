  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto rounded-[32px] p-0 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl dark:shadow-none flex flex-col">
        <DialogHeader className="p-8 pb-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 sticky top-0 z-10 backdrop-blur-md">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-950 flex items-center justify-center border border-blue-200 dark:border-blue-900">
        ...
        <div className="p-8 space-y-6 flex-1 overflow-y-auto">
          {announcements.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-12 space-y-4">
              <div className="w-16 h-16 rounded-full bg-slate-50 dark:bg-slate-950 flex items-center justify-center border border-dashed border-slate-200 dark:border-slate-800">
                <Bell className="w-6 h-6 text-slate-300 dark:text-slate-700" />
              </div>
              <div>
                <p className="font-bold text-slate-800 dark:text-slate-200 text-sm">No active announcements</p>
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Check back later for studio updates.</p>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              {announcements.map((ann) => {
                const isRead = ann.readBy?.includes(authTrainer.id || "") ?? false;
                ...
                return (
                  <div
                    key={ann.id}
                    className={cn(
                      "p-6 rounded-3xl border transition-all relative overflow-hidden bg-white dark:bg-slate-900",
                      isRead
                        ? "border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700"
                        : "border-blue-200 dark:border-blue-900 bg-gradient-to-r from-blue-50/20 to-transparent dark:from-blue-950/10 shadow-sm"
                    )}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-2 flex-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="font-black text-slate-900 dark:text-white italic tracking-tight text-lg">
                            {ann.title}
                          </h3>
                          {ann.priority === "high" && (
                        ...
                        {ann.longContent && (
                          <div className="text-xs text-slate-500 dark:text-slate-400 mt-3 pt-3 border-t border-dashed border-slate-100 dark:border-slate-800 leading-relaxed whitespace-pre-wrap">
                            {ann.longContent}
                          </div>
                        )}
        ...
        <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-end">
          <Button
            ...
            className="rounded-2xl font-bold uppercase tracking-widest text-xs h-11 px-6 bg-slate-900 hover:bg-slate-800 text-white dark:bg-slate-100 dark:hover:bg-slate-200 dark:text-slate-950"
          >
            Acknowledge & Close
          </Button>
        </div>
